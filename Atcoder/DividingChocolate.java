...
public class DividingChocolate {
	...
	public static void main(String[] args) {
        ...
        for(int i=0;i<H;i++){
        	// H : 수평선 , W : 수직선
        	// 0의 부분 : 검은색 초콜릿, 1의 부분 : 흰색 초콜릿
            char[] tmp = sc.next().toCharArray(); 
            // 문자열을 array로 변경 (각각의 초콜릿 조각의 색을 입력)
            for(int j=0;j<W;j++){
                choco[i][j] = tmp[j] - '0';
                System.out.println(i + "행 " + j + 
                		"열 의 초콜릿의 값 " + choco[i][j]);
            }
        }
        int[][] sum = new int[H+1][W];
        for(int i=1 ; i<=H ; i++){ // 상위 choco for문과 동일
            for(int j=0;j<W;j++){ // W : 수직선
                sum[i][j] = sum[i-1][j] + choco[i-1][j];
                //sum[1][0] = sum[0][0] + choco[0][0];
                //sum[1][1] = sum[0][1] + choco[0][1];
                //sum[1][2] = sum[0][2] + choco[0][2];
                //sum[1][3] = sum[0][3] + choco[0][3];
                //sum[1][4] = sum[0][4] + choco[0][4];
                //sum[2][0] = sum[1][0] + choco[1][0];
                //sum[2][1] = sum[1][1] + choco[1][1];
                //sum[2][2] = sum[1][2] + choco[1][2];
                //sum[2][3] = sum[1][3] + choco[1][3];
                //sum[2][4] = sum[1][4] + choco[1][4];
        ...
        loop : for(int state=0;state<(1<<H-1);state++){
            ArrayList<Integer> sep = new ArrayList<>();
            sep.add(0); // ArrayList에 0의 값을 추가
            for(int i=0;i<H;i++){
                if((state & (1<<i)) != 0){
                    sep.add(i+1);
                }
            ...
            int[] white = new int[sep.size()-1];
            // for(int i=0;i<sep.size()-1;i++){
            //     for(int k=sep.get(i);k<sep.get(i+1);k++){
            //         white[i] += choco[k][0];
            //     }
            //     if(white[i] > K){
            //         continue loop;
            ...
            for(int j=0;j<W;j++){
                boolean sup = false;
                for(int i=0;i<sep.size()-1;i++){
                    if(white[i] + sum[sep.get(i+1)][j] - sum[sep.get(i)][j] > K){
                        sup = true;
                    }
                }
                for(int i=0;i<sep.size()-1;i++){
                    if(!sup){
                        white[i] += sum[sep.get(i+1)][j] - sum[sep.get(i)][j];
                    } else {
                        white[i] = sum[sep.get(i+1)][j] - sum[sep.get(i)][j];
                        if(white[i] > K){
                            continue loop;
                ...
                // out.println("state = "+Integer.toBinaryString(state)+ ", j = " + j +": white"+ Arrays.toString(white));
            }
