...
public class LineCodingTest2_final {
		...
		// TODO Auto-generated method stub
			...
			for(int b = 0 ; b < sheets.length ; b++) {
				for(int c = b+1 ; c < sheets.length ; c++) {
					for(int k = 0 ; k < answer_sheet.length()-1; k++) {
					...
					System.out.printf("%s번 학생과 %s번 학생의 부정문항 점수 : %s\t 부정연속 점수 : %s\n",b,c,negnum,negpos);
					total = negnum+(negpos*negpos);
					if(total >= max) {
						max = total;
					}
					negnum = 0;
				...
				System.out.printf("부정문항 지표 최대값 : %s\n",max);
			}				
}
}
